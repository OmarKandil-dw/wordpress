jQuery(document).ready(function($) {
    'use strict';
    var this_obj = affiliate_review_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.affiliate-review-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message affiliate-review-plugin-install');
                button.addClass('button-primary affiliate-review-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.affiliate-review-recommended-plugins .affiliate-review-activation-note').css('display','block');
    ...
    $(document).on('click', '.affiliate-review-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .affiliate-review-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .affiliate-review-recommended-plugins').css('display','none');
    });
