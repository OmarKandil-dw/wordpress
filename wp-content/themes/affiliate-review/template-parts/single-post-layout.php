...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
    <h1><?php the_title(); ?></h1>
    <?php if( get_theme_mod( 'affiliate_review_toggle_postdate',true) != '' || get_theme_mod( 'affiliate_review_toggle_author',true) != '' || get_theme_mod( 'affiliate_review_toggle_comments',true) != '' || get_theme_mod( 'affiliate_review_toggle_time',true) != '') { ?>
        <div class="post-info">
            <hr>
    ...
    <?php if(get_theme_mod('affiliate_review_toggle_tags',true)==1){ ?>
    ...
    <?php } ?>
        ...
        if ( is_singular( 'attachment' ) ) {
            ...
            the_post_navigation( array(
                'prev_text' => _x( '<span class="meta-nav">Published in</span><span class="post-title">%title</span>', 'Parent post link', 'affiliate-review' ),
            ) );
        } elseif ( is_singular( 'post' ) ) {
            // Previous/next post navigation.
            the_post_navigation( array(
                'next_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Next', 'affiliate-review' ) . '</span> ' .
                    '<span class="screen-reader-text">' . __( 'Next post:', 'affiliate-review' ) . '</span> ' .
                    '<span class="post-title">%title</span>',
                'prev_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Previous', 'affiliate-review' ) . '</span> ' .
                    '<span class="screen-reader-text">' . __( 'Previous post:', 'affiliate-review' ) . '</span> ' .
                    '<span class="post-title">%title</span>',
            ) );
        }
    ?>
